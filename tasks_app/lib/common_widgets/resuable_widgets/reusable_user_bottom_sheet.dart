...
void showUserAddTaskBottomSheet({
  ...
  // Add "اختر" as the first item in the dropdown lists
  final List<String> appNamesWithSelect = ['اختر', ...appNames];
  final List<String> placeNamesWithSelect = ['اختر', ...placeNames];

  // Determine initial values - ALWAYS use 'اختر' as default
  final String validInitialAppName = 'اختر';
  final String validInitialPlaceName = 'اختر';

  // Debug print to verify
  print('DEBUG: validInitialAppName = $validInitialAppName');
  print('DEBUG: validInitialPlaceName = $validInitialPlaceName');
  print('DEBUG: appNamesWithSelect = $appNamesWithSelect');
  print('DEBUG: placeNamesWithSelect = $placeNamesWithSelect');
  ...
  showModalBottomSheet(
    ...
    builder: (bottomSheetContext) => StatefulBuilder(
      builder: (builderContext, setState) {
        return _buildBottomSheetContent(
          context: context,
          appNames: appNamesWithSelect,
          placeNames: placeNamesWithSelect,
          coOperatorUsers: coOperatorUsers,
          initialTaskTitle: initialTaskTitle,
          initialAppName: validInitialAppName,
          initialPlaceName: validInitialPlaceName,
...
class _UserAddTaskBottomSheetContentState
  ...
  void initState() {
    ...
    for (var field in widget.fields) {
      if (field is TextFieldConfig) {
        ...
        // CRITICAL FIX: Directly assign the initial value without any validation
        // The initial value has already been validated at the parent level
        String? valueToSet = field.initialValue;

        // If for some reason initialValue is null, use the first item
        if (valueToSet == null && field.items.isNotEmpty) {
          valueToSet = field.items.first;
        }

        // Debug print to verify
        print('DEBUG: Setting dropdown ${field.key} to "$valueToSet"');
        print('DEBUG: Available items: ${field.items}');

        _dropdownValues[field.key] = valueToSet;
      } else if (field is MultiSelectDropdownFieldConfig) {
        final validInitialValues = field.initialValues
                ?.where((v) => field.items.contains(v))
                .toList() ??
  ...
  Widget _buildField(FieldConfig field, bool isDark, ColorScheme colorScheme) {
    if (field is TextFieldConfig) {
      ...
      // CRITICAL FIX: Get the value directly without any modification
      final currentValue = _dropdownValues[field.key];

      // Debug print to see what's happening
      print(
          'DEBUG: Building dropdown ${field.key}, currentValue = "$currentValue"');
      print('DEBUG: Items available: ${field.items}');
      ...
      return Padding(
        ...
        child: DropdownButtonFormField<String>(
          ...
          items: field.items
              ...
              .toList(),
          onChanged: (value) {
            print('DEBUG: Dropdown ${field.key} changed to "$value"');
            setState(() {
              _dropdownValues[field.key] = value;
            });
            field.onChanged?.call(value);
  ...
  void _handleSubmit() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      _controllers.forEach((key, controller) => values[key] = controller.text);
      _dropdownValues.forEach((key, value) {
        // Don't include 'اختر' in the submitted values
